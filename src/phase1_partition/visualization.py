...
def plot_partition(G, pos, node_to_community, num_communities, save_path):
    """
    Plot network partition graph and save to file with legend.
    ...
    # Draw nodes by community and create legend handles
    legend_handles = []
    ...
    # Add legend (adjust columns based on community count)
...
def save_all_partitions_plots(G, pos, unique_partitions, output_dir='partition_plots'):
    ...
    Args:
        G: NetworkX graph
        pos: Node positions dictionary
        unique_partitions: Dict of {num_communities: {'node_to_community': dict, 'resolution': float}}
